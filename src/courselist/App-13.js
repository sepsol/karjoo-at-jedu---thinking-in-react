...
// it worked! now let's come up with a way like addCourseToList() function inside removeCourse()
...
function App(props) {
    ...
    function removeCourse() {                           // to remove singular items from the list
        courseList.splice(0, 1);                        // there's a method called array.splice(), let's see what that does
        setCourseList([...courseList]);                 // now let's set the courseList to the new courseList array, we can see that this is always removing the top item from the list
            ...
            <ul>
                {
                    courseList.map(                     // .map is a method that we can run on arrays like courseList. it creates a loop and calles each item in the array recursively
                        (item, index) => 
                            <li>
                                {index} {item}
                                <span onClick={removeCourse}> X</span>
                            </li>                       // let's create an 'X' button using html <span>
